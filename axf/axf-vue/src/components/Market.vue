...
export default {
  ...
  methods:{
    fetchMarket: function(){
      ...
      this.axios.get('http://127.0.0.1:8000/axf/market/',{
        params
      }).then(response => {
    ...
    addShopping: function (item){
      ...
      this.axios.get('http://127.0.0.1:8000/axf/carts/',{
          params
      }).then(response => {
        const resp = response.data;
        if(resp.status == 200) {
  ...
  mounted: function (){
    ...
    this.axios.get('http://127.0.0.1:8000/axf/foodtypes/').then(response => {
      const resp = response.data;
      if(resp.status == 200) {
        const data = resp.data;
        this.foodtypes = data;
